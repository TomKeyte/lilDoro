...
const Canvas = styled.canvas`border-radius: 5px;`
...
export default class NumberLine extends React.Component {
	...
	draw() {
		...
		let majorTop = 0.5 * this.cHeight - 0.5 * this.majorHeight
		let minorTop =
			0.5 * this.cHeight + 0.5 * this.majorHeight - this.minorHeight

		// width of 1 minutes
		let minuteWidth =
			this.majorWidth + 4 * this.minorWidth + 5 * this.margin

		let startMins = this.state.start / 60
		let currentMins = this.state.current / 60
		let elapsedMins = startMins - currentMins
		let xStart =
			this.cWidth / 10 - (numMins - elapsedMins) * minuteWidth / 5
		let x, i

		// i is number of minutes
		for (i = 0; i <= numMins; i++) {
			c.beginPath()
			if (i % 5 === 0) {
				let min = i / 5
				...
				if (
					...
					c.fillText(
						i,
						xStart + min * minuteWidth - adj + 0.5,
						majorTop + this.majorHeight + 15
					)
				}
			} else {

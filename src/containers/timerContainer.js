...
export default class TimerContainer extends Component {
	...
	_handlePlay() {
		...
		play && !this.interval && (this.interval = setInterval(this.tick, 1000))
	}
	...
	tick() {
		...
		if (this.state.play) {
			...
			this.setState({ time })
			if (this.state.tick) {
				let tickSound = new Audio(tick)
				tickSound.volume = this.state.volume / 100
				tickSound.play()
				setTimeout(() => tickSound.pause(), 1000)
			}
		}
	}
